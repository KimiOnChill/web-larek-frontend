...
В полях класса хранятся следующие данные:
- `itemsList: IItem[]` - массив объектов выбранных товаров.
...
- `set gallery(items: HTMLElement[])` - сеттер для получения массива товаров.
...
- `set content(items: HTMLElement)` - сеттер для разметки контента.
...
- `formInputs: NodeListOf<HTMLInputElement>` - список всех полей ввода формы.
- `formErrors: Record<string>` - объект для вывода ошибок.
...
- `formErrors:changed` - изменение в списке ошибок валидации формы.

*События, возникающие при взаимодействии пользователя с интерфейсом (генерируются классами, отвечающими за представление)*
- `modal:open` - открытие модального окна.
- `modal:close` - закрытие модального окна.
- `basket:open` - открытие корзины.
- `item:select` - выбор карточки.
...
- `${form}.${field}:changed` - изменение поля в форме.